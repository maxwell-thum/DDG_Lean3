...
lemma mem_vertices : x ∈ K.vertices ↔ {x} ∈ K.faces := abstract_simplicial_complex.mem_vertices

lemma vertices_eq : K.vertices = ⋃ k ∈ K.faces, (k : set E) := abstract_simplicial_complex.vertices_eq
-/

lemma vertices_subset_space : K.vertices ⊆ K.space :=
abstract_simplicial_complex.vertices_eq.subset.trans $ Union₂_mono $ λ x hx, subset_convex_hull 𝕜 x
...
instance : semilattice_inf (K : simplicial_complex 𝕜 E) :=
{ inf := (⊓),
  inf_le_left := λ K L s hs, hs.1,
  inf_le_right := λ K L s hs, hs.2,
  le_inf := λ K L M hKL hKM s hs, ⟨hKL hs, hKM hs⟩,
  .. (partial_order.lift (abstract_simplicial_complex.faces ∘ to_abstract_simplicial_complex) $ λ x y, ext _ _) }
-/
...
lemma facets_bot : (⊥ : simplicial_complex 𝕜 E).facets = ∅ := eq_empty_of_subset_empty abstract_simplicial_complex.facets_subset
